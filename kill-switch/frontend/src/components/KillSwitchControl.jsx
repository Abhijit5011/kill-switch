...
export default function KillSwitchControl({ isFrozen, onOpenModal }) {
  ...
  return (
    <motion.div 
      ...
      className={`glass-panel rounded-2xl p-6 flex flex-col items-center text-center transition-all duration-300 relative overflow-hidden ${
        isFrozen ? 'border-red-500/60 bg-red-950/20 shadow-[0_0_30px_rgba(239,68,68,0.15)]' : ''
      }`}
    >
        ...
        >
          <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
            ...
            <motion.span 
              ...
              className={`text-3xl mb-1 drop-shadow-md cover-lock-icon transition-colors duration-200 ${
                isHovered || isFrozen ? 'text-[#EF4444]' : 'text-[#F5F5F5]'
              }`}
            >
              🔒
            </motion.span>
            ...
            <motion.span 
              ...
              className={`text-[10px] font-semibold mt-1 font-mono transition-colors duration-200 ${
                isHovered || isFrozen ? 'text-[#EF4444]' : 'text-[#8A8A8E]'
              }`}
            >
      ...
      <motion.button
        ...
        disabled={!isFrozen}
        className={`w-full py-3 rounded-xl font-mono text-xs font-bold transition-all border ${
          isFrozen

...
export default function N8nPipelineVisualizer({ 
  ...
  useEffect(() => {
    if (!isSending) {
      if (lastDecision) {
        // Keep final subnode highlighted on completion
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-[#2A2A2A] pb-4">
        <div>
          ...
          <p className="text-xs text-[#8A8A8E] mt-1">Full internal node graph tracing across Workflows A, B, C, and D</p>
        </div>
      ...
      {/* Main Workflow Topology Canvas with Internal Sub-Nodes */}
      <div className="space-y-4 relative py-2">
        ...
        {(() => {
          ...
          return (
            <div className={`p-4 rounded-xl border border-[#2A2A2A] bg-[#141414] space-y-3 ${isFrozen ? 'border-red-500/60 bg-red-950/20' : ''}`}>
